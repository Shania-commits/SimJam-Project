...
namespace PassthroughCameraSamples.StartScene
{
    ...
    public class StartMenu : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            var simJamScenes = new List<Tuple<int, string>>();
            ...
            for (var sceneIndex = 1; sceneIndex < n; ++sceneIndex)
            {
                ...
                if (path.Contains("Assets/Simulation"))
                {
                    simJamScenes.Add(new Tuple<int, string>(sceneIndex, path));
                }
            ...
            if (simJamScenes.Count > 0)
            {
                _ = uiBuilder.AddDivider(DebugUIBuilder.DEBUG_PANE_CENTER);
                _ = uiBuilder.AddLabel("SimJam Scenes", DebugUIBuilder.DEBUG_PANE_CENTER);
                foreach (var scene in simJamScenes)
                {
